...
public class UniGifImage : MonoBehaviour
{
    ...
    public IEnumerator SetGifFromUrlCoroutine(string url, bool autoPlay = true)
    {
        ...
        using (WWW www = new WWW(path))
        {
            ...
            Clear();
            m_rawImage.color = new Color(m_rawImage.color.r, m_rawImage.color.g, m_rawImage.color.b, 0f);
            nowState = State.Loading;
            // Get GIF textures
            yield return StartCoroutine(UniGif.GetTextureListCoroutine(www.bytes, (gifTexList, loopCount, width, height) =>
            {
                if (gifTexList != null)
                {
                    ...
                    this.width = width;
                    this.height = height;
                    m_rawImage.color = new Color(m_rawImage.color.r, m_rawImage.color.g, m_rawImage.color.b, 1f);
